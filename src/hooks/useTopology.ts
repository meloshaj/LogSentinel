...
// ---------------------------------------------------------------------------
// URL candidates (mirrors TelemetryProvider strategy)
// ---------------------------------------------------------------------------

function buildTopologyUrls(): string[] {
  const candidates = [
    import.meta.env.VITE_API_URL
      ? `${(import.meta.env.VITE_API_URL as string).replace(/\/$/, "")}${TOPOLOGY_PATH}`
      : undefined,
    `${window.location.origin}${TOPOLOGY_PATH}`,
    FALLBACK_URL,
  ];
...
// ---------------------------------------------------------------------------
// Defensive mappers — backend returns dict[str, Any], we normalise safely
// ---------------------------------------------------------------------------
...
const VALID_EDGE_STATUSES = new Set<EdgeStatus>([
  "normal",
  "stressed",
  "failing",
]);
...
function mapNode(raw: Record<string, unknown>, index: number): TopologyNode {
  ...
  const type: NodeType = VALID_NODE_TYPES.has(rawType as NodeType)
    ? (rawType as NodeType)
    : "service";

  const rawStatus =
    typeof raw.status === "string" ? raw.status.toLowerCase() : "";
  const status: NodeStatus = VALID_NODE_STATUSES.has(rawStatus as NodeStatus)
...
function mapEdge(raw: Record<string, unknown>, index: number): TopologyEdge | null {
  ...
  const id =
    typeof raw.id === "string" && raw.id ? raw.id : `edge-${source}-${target}-${index}`;

  const rawStatus =
    typeof raw.status === "string" ? raw.status.toLowerCase() : "";
  const status: EdgeStatus | undefined = VALID_EDGE_STATUSES.has(
...
export function useTopology(): UseTopologyResult {
  ...
  useEffect(() => {
    fetchTopology();
    const timer = setInterval(fetchTopology, POLL_INTERVAL_MS);
    return () => {
      clearInterval(timer);
      abortRef.current?.abort();
    };
  ...
  return { nodes, edges, updatedAt, isLoading, error, refresh: fetchTopology };
}
