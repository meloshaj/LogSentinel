...
function LogHistoryTable({
  ...
  const levelBg: Record<string, string> = {
    INFO: "rgba(31,111,235,0.15)",
    WARN: "rgba(210,153,34,0.15)",
    ERROR: "rgba(218,54,51,0.15)",
    DEBUG: "rgba(125,133,144,0.1)",
  };
  ...
  return (
    <div className="rounded-xl bg-[#161b22] border border-[#21262d] overflow-hidden">
      <div className="flex items-center justify-between px-4 py-3 border-b border-[#21262d]">
        ...
        <button
          disabled
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[#21262d] text-[#7d8590] opacity-50 cursor-not-allowed"
          style={{ fontSize: "11px" }}
        >
          ...
          <thead>
            <tr className="border-b border-[#21262d]">
              {["Timestamp", "Level", "Service", "Message"].map((h) => (
                <th
                  key={h}
                  className="px-4 py-2.5 text-left text-[#484f58]"
                  style={{
                    fontSize: "10px",
                    fontWeight: 600,
          ...
          <tbody className="divide-y divide-[#21262d]/50">
            {rows.map((log) => (
              <tr key={log.id} className="hover:bg-[#21262d]/30 transition-colors">
                <td
                  className="px-4 py-2.5 text-[#484f58]"
                  style={{ fontSize: "11px", fontFamily: "monospace", whiteSpace: "nowrap" }}
                >
                  {log.timestamp}
                </td>
                ...
                <td
                  className="px-4 py-2.5 text-[#7d8590]"
                  style={{ fontSize: "11px", fontFamily: "monospace", whiteSpace: "nowrap" }}
                >
                  {log.service}
                </td>
                <td
                  className="px-4 py-2.5 text-[#c9d1d9]"
...
export function LogsPage() {
  ...
  return (
    <div className="space-y-4">
      {/* Page header */}
      <div>
        <h1 className="text-[#e6edf3]" style={{ fontSize: "18px", fontWeight: 700 }}>
          Live Logs
        </h1>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <button
          disabled
          className="flex items-center gap-2 px-3 py-2 rounded-lg bg-[#161b22] border border-[#21262d] text-[#7d8590] opacity-50 cursor-not-allowed"
          style={{ fontSize: "12px" }}
        >
      ...
      <div style={{ height: 400 }}>
        <LogStream serviceFilter={service} searchQuery={search} />
      </div>

      {/* Log history table */}
      <LogHistoryTable serviceFilter={service} searchQuery={search} />
    </div>
  );
}
