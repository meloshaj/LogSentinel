...
class BenchmarkingCollector:
    ...
    def record_latency(self, latency_ms: float):
        ...
        if self._ema_latency_ms == 0.0:
            ...
            self._ema_latency_ms = (self.ema_alpha * latency_ms) + (
                (1 - self.ema_alpha) * self._ema_latency_ms
            )

        if self._ema_latency_ms > self.latency_threshold_ms:
            self._trigger_event(
                "pipeline_latency", self._ema_latency_ms, self.latency_threshold_ms
            )
        self._publish_observability()
    ...
    def set_queue_depth(self, depth: int, max_depth: int = 10000):
        ...
        if ratio > self.queue_depth_threshold_ratio:
            self._trigger_event(
                "queue_backpressure", ratio, self.queue_depth_threshold_ratio
            )
        self._publish_observability()
    ...
    def record_db_batch_duration(self, duration_ms: float):
        ...
        if self._ema_db_batch_ms == 0.0:
            ...
            self._ema_db_batch_ms = (self.ema_alpha * duration_ms) + (
                (1 - self.ema_alpha) * self._ema_db_batch_ms
            )

        if self._ema_db_batch_ms > self.db_batch_threshold_ms:
            self._trigger_event(
                "db_batch_delay", self._ema_db_batch_ms, self.db_batch_threshold_ms
            )
        self._publish_observability()
    ...
    def get_health_metrics(self) -> dict[str, Any]:
        ...
        self._update_throughput_window()
        throughput = (
            sum(self._throughput_window) / max(1, len(self._throughput_window))
        ...
        return {
            ...
            "queue_capacity_percent": round(
                (self._queue_depth / max(1, self._max_queue_depth)) * 100, 2
            ),
    ...
    def _trigger_event(self, metric_name: str, current_value: float, threshold: float):
        ...
        event = PerformanceEvent(
            ...
            threshold=threshold,
            severity="error"
            if "delay" in metric_name or "backpressure" in metric_name
            else "warning",
