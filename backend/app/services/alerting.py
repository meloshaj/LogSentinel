...
async def _coalesce_and_dispatch(
    ...
    try:
        ...
        if not raw_items:
            logger.warning("No anomalies found in buffer %s after window elapsed", buffer_key)
            return
            
        # Parse items
        payloads = [IncidentAlertPayload.model_validate_json(item) for item in raw_items]
...
def _coalesce_payloads(tracking_id: str, payloads: list[IncidentAlertPayload]) -> IncidentAlertPayload:
...
async def _send_discord(client: httpx.AsyncClient, url: str, payload: IncidentAlertPayload) -> None:
    ...
    chain_str = " ➔ ".join(payload.propagation_chain) if payload.propagation_chain else "Unknown"
...
async def _send_slack(client: httpx.AsyncClient, url: str, payload: IncidentAlertPayload) -> None:
