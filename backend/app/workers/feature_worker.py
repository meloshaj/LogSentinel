...
class FeatureExtractionWorker:
                ...
                try:
                    ...
                    try:
                        if (
                            ...
                            and self.anomaly_detector.model is not None
                        ):
                        ...
                        logger.exception(
                            "Failed to run anomaly prediction for window %s",
                            window.window_id,
                        )
    ...
    def _resolve_anomaly_detector(
        ...
        try:
            ...
            logger.exception(
                "Failed to load Isolation Forest artifact from %s", model_path
            )
            return None
    ...
    def _schedule_telemetry_event(self, event: dict[str, Any]) -> None:
        try:
            ...
            logger.debug(
                "No running event loop available for feature telemetry broadcast"
            )
