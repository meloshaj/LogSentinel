...
def map_severity_number(severity_number: int | None, severity_text: str | None) -> str:
    ...
    if severity_text:
        ...
        if text_upper in ("TRACE", "DEBUG"):
            return "DEBUG"
        if text_upper == "INFO":
            return "INFO"
        if text_upper == "WARN":
            return "WARN"
        if text_upper in ("ERROR", "FATAL", "CRITICAL"):
            return "ERROR"
...
def parse_otel_time(
    time_unix_nano: str | None, observed_time_unix_nano: str | None
) -> str:
...
async def ingest_logs(
    ...
    for resource_log in payload.resource_logs:
        resource_attrs = (
            extract_attributes(resource_log.resource.attributes)
                    ...
                    "logs": [
                        {
                            ...
                            "level": map_severity_number(
                                record.severity_number, record.severity_text
                            ),
                            "message": log_message,
                            "metadata": metadata,
                        }
