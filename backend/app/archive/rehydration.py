...
class RehydrationRequest(BaseModel):
    archive_ids: list[str] = Field(..., description="List of archive UUIDs to rehydrate")
...
async def rehydrate_archives(
    ...
    async with engine.connect() as conn:
        result = await conn.execute(
            text("SELECT archive_id, object_key FROM archive_manifest WHERE archive_id = ANY(:ids)"),
    ...
    return RehydrationResponse(
        ...
        expires_at=datetime.utcnow() # In a real implementation this would be linked to pg_cron or similar
    )
