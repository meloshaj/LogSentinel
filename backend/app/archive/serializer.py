...
def serialize_to_parquet(rows: list[dict[str, Any]]) -> tuple[bytes, dict[str, Any]]:
    ...
    arrays = [pa.array(columns[field.name], type=field.type) for field in ARCHIVE_SCHEMA]
...
async def async_serialize_to_parquet(rows: list[dict[str, Any]]) -> tuple[bytes, dict[str, Any]]:
