...
class ArchiveWorker:
    ...
    async def _step_create_manifests(self) -> None:
        ...
        engine = get_engine()
        retention_threshold = datetime.now(timezone.utc) - timedelta(days=self.settings.archive_retention_days)
        ...
        async with engine.connect() as conn:
            result = await conn.execute(query, {"retention_threshold": retention_threshold})
    ...
    async def _step_process_state_machine(self) -> None:
        ...
        async with engine.connect() as conn:
            result = await conn.execute(lease_query, {"owner": self.instance_id, "expires": lease_expires})
        ...
        try:
            ...
            logger.error("Error processing archive %s (status %s): %s", archive_id, status, e, exc_info=True)
            # Release lease on error
            async with engine.connect() as conn:
                await conn.execute(
                    text("UPDATE archive_manifest SET lease_owner = NULL WHERE archive_id = :id"),
                    {"id": archive_id}
                )
                await conn.commit()
    ...
    async def _transition_hot_to_exporting(self, record: dict) -> None:
        ...
        async with engine.connect() as conn:
            await conn.execute(
                text("UPDATE archive_manifest SET status = 'EXPORTING' WHERE archive_id = :id"),
    ...
    async def _transition_exporting_to_stored(self, record: dict) -> None:
        ...
        # Write to S3
        self.s3_client.put_if_absent(record["object_key"], parquet_bytes, "application/vnd.apache.parquet")
        
        # Update record with stats to generate correct sidecar
        record.update(stats)
        sidecar_bytes = generate_sidecar_manifest(record)
        self.s3_client.put_if_absent(record["sidecar_key"], sidecar_bytes, "application/json")
    ...
    async def _transition_stored_to_verified(self, record: dict) -> None:
        ...
        async with engine.connect() as conn:
            if is_valid:
                await conn.execute(
                    text("UPDATE archive_manifest SET status = 'VERIFIED', verified_at = NOW() WHERE archive_id = :id"),
                    {"id": record["archive_id"]}
                )
            else:
                await conn.execute(
                    text("UPDATE archive_manifest SET status = 'CORRUPT' WHERE archive_id = :id"),
                    {"id": record["archive_id"]}
                )
            await conn.commit()
    ...
    async def _transition_verified_to_hot_deleted(self, record: dict) -> None:
        ...
        async with engine.connect() as conn:
            ...
            await conn.execute(
                text("UPDATE archive_manifest SET status = 'HOT_DELETED', deleted_from_hot_at = NOW(), completed_at = NOW() WHERE archive_id = :id"),
                {"id": record["archive_id"]}
            )
            await conn.commit()
