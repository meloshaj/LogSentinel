...
from backend.app.archive.s3_client import S3StorageClient
import logging
...
class ArchiveVerifier:
    ...
    def verify_archive(self, manifest_record: dict) -> bool:
        ...
        if not stream:
            logger.error("Archive verification failed: Object %s not found in S3", object_key)
            return False
        ...
        if actual_sha256 != expected_sha256:
            logger.error(
                ...
                object_key, expected_sha256, actual_sha256
            )
            return False
        ...
        try:
            ...
            logger.error("Archive verification failed: Could not parse Parquet file %s: %s", object_key, e)
            return False
            
        if actual_row_count != expected_row_count:
            logger.error(
                "Archive verification failed: Row count mismatch for %s. Expected %s, got %s",
                object_key, expected_row_count, actual_row_count
            )
            return False
