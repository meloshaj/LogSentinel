...
async def run_benchmark(batch_sizes: list[int], total_records: int) -> None:
    ...
    print(f"Executing pre-flight database connection check to {settings.host}:{settings.port}...")
    try:
    ...
    print(f"{'Batch Size':<15} | {'Total Duration (s)':<20} | {'Records/Sec':<15} | {'p95 Latency (ms)'}")
    print("-" * 75)
    ...
    for batch_size in batch_sizes:
        ...
        records_per_sec = summary.get("batches", {}).get("throughput_records_per_sec", 0.0)
        p95_latency = summary.get("batches", {}).get("p95_duration_ms", 0.0)
        
        print(f"{batch_size:<15} | {total_duration_s:<20.4f} | {records_per_sec:<15.2f} | {p95_latency:.2f}")
