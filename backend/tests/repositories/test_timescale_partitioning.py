...
MIGRATION_PATH = REPO_ROOT / "scripts" / "migrations" / "20260826_0001_multitenant_partitioning.sql"
...
def test_logs_primary_key_includes_tenant_and_ingested_at():
    ...
    match = re.search(r"CREATE TABLE[\s\S]*?logs\s*\([\s\S]*?PRIMARY KEY\s*\((.*?)\)", MIGRATION_SQL, re.IGNORECASE)
...
def test_logs_hypertable_chunk_interval():
    ...
    match = re.search(r"create_hypertable\('logs',\s*'ingested_at',\s*chunk_time_interval\s*=>\s*INTERVAL\s*'1 day'", MIGRATION_SQL, re.IGNORECASE)
    assert match is not None, "Could not find create_hypertable with chunk_time_interval 1 day on ingested_at"
