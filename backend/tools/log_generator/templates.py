...
def _build_template_vars(rng: random.Random) -> dict[str, Any]:
    ...
    return {
        ...
        "upstream": rng.choice(
            ["auth-service", "order-service", "payment-gateway", "inventory-db"]
        ),
        ...
        "config_key": rng.choice(
            ["log_level", "pool_size", "cache_ttl", "rate_limit", "feature_flags"]
        ),
        "mem_pct": rng.randint(75, 97),
        "error_class": rng.choice(
            [
                "ValueError",
                "RuntimeError",
                "KeyError",
...
def render_log(
    ...
    variables["duration_ms"] = _random_duration_ms(
        base_latency_ms, latency_jitter_ms, rng
    )
